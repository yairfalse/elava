...
func TestWAL_AppendAndRead(t *testing.T) {
	...
	// Verify entries
	verifyEntries(t, reader, resource.ID)
}

func verifyEntries(t *testing.T, reader *Reader, resourceID string) {
	...
	for i, expected := range expectedTypes {
		...
		if entry.ResourceID != resourceID {
			t.Errorf("Entry %d: resource_id = %v, want %v", i, entry.ResourceID, resourceID)
		}
...
func TestWAL_Replay(t *testing.T) {
	...
	_ = w.Append(EntryObserved, "old-resource", map[string]string{"age": "old"})
	...
	_ = w.Append(EntryObserved, "new-resource-1", map[string]string{"age": "new"})
	_ = w.Append(EntryObserved, "new-resource-2", map[string]string{"age": "new"})
...
func TestWAL_SequenceNumbers(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_ = w.Append(EntryObserved, fmt.Sprintf("resource-%d", i), nil)
	}
...
func TestWAL_DataIntegrity(t *testing.T) {
	...
	_ = w.Append(EntryDecided, decision.ResourceID, decision)
	_ = w.Close()
